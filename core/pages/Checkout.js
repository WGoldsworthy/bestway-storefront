...
export default {
  ...
  methods: {
    ...
    checkTobacco () {
      var parent = this
      var response = true
      Array.prototype.forEach.call(this.$store.state.cart.cartItems, function (item) {
        if (item.extension_attributes.category_links[0].category_id === '21') {
          console.log(parent.$store.state.user.current)
          if (parent.$store.state.user.current.group_id === 4) {
          } else {
            response = false
          }
        }
    ...
    placeOrder () {
      ...
      if (this.checkStocks()) {
        ...
        // TODO: This is hack bodged for DEMO ONLY. RETURN THIS TO this.notifyNotAvailable();
        if (this.checkTobacco()) {
          this.$store.dispatch('checkout/placeOrder', { order: this.prepareOrder() })
        } else {
          this.notifyTobacco()
        }
