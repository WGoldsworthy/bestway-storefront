<template>
  <div class="header">
    <header
      class="fixed w-100 brdr-bottom-1 bg-cl-primary brdr-cl-secondary"
      :class="{ 'is-visible': navVisible }"
    >
      <!-- <top-header/> -->
      <div class="container px15">
        <div class="row between-xs middle-xs" v-if="!isCheckoutPage">
          <div class="col-md-4 col-xs-2 middle-xs">
            <div>
              <template v-if="!canGoBack">
                <hamburger-icon class="p15 icon bg-cl-secondary pointer" v-if="!canGoBack"/>
              </template>
              <template v-else>
                <return-icon class="p15 icon bg-cl-secondary pointer" v-if="canGoBack"/>
              </template>
            </div>
          </div>
          <!-- <div class="col-xs-2 visible-xs">
            <search-icon class="p15 icon pointer" />
          </div> -->
          <div class="col-md-4 col-xs-4 center-xs pt5 mobile-logo">
            <div>
              <logo width="auto" height="35px"/>
            </div>
          </div>
          <!-- <div class="col-xs-2 visible-xs">
            <wishlist-icon class="p15 icon pointer" />
          </div> -->
          <div class="inline-flex right-icons visible-xs">
            <button
              type="button"
              @click="isMenuOpen = !isMenuOpen"
              :class="!isMenuOpen ? 'chevron-down' : 'chevron-up'"
            >
            </button>
          </div>
          <div
            v-if="isMenuOpen"
            class="icons-container visible-xs"
          >
            <div
              class="icons-holder"
              @click="closeIconMenu"
            >
              <account-icon class="icon pointer" />
              <wishlist-icon class="icon pointer" />
              <div
                v-if="currentUser"
                class="bg-cl-transparent brdr-none inline-flex p15 icon pointer"
              >
                <microcart-icon class="icon pointer" />
              </div>
              <search-icon class="icon pointer" />
            </div>
            <span @click="closeIconMenu"></span>
          </div>
          <div class="col-md-4 col-xs-2 end-xs">
            <div class="inline-flex right-icons">
              <search-icon class="p15 icon hidden-xs pointer" />
              <wishlist-icon class="p15 icon hidden-xs pointer" />
              <compare-icon class="p15 icon hidden-xs pointer" />
              <div
                v-if="currentUser"
                class="micro-cart"
              >
                <microcart-icon class="p15 icon pointer" />
              </div>
              <account-icon class="p15 icon hidden-xs pointer" />
            </div>
          </div>
        </div>
        <div class="row between-xs middle-xs px15 py5" v-if="isCheckoutPage">
          <div class="col-xs-5 col-md-3 middle-xs">
            <div>
              <router-link :to="localizedRoute('/')" class="cl-tertiary links">
                {{ $t('Return to shopping') }}
              </router-link>
            </div>
          </div>
          <div class="col-xs-2 col-md-6 center-xs">
            <logo width="auto" height="41px"/>
          </div>
          <div class="col-xs-5 col-md-3 end-xs">
            <div>
              <a v-if="!currentUser" href="#" @click.prevent="gotoAccount" class="cl-tertiary links">
                {{ $t('Login to your account') }}
              </a>
              <span v-else>
                {{ $t('You are logged in as') }} {{ currentUser.firstname }}
              </span>
            </div>
          </div>
        </div>
      </div>
    </header>
    <div class="header-placeholder"/>
  </div>
</template>

<script>
import { mapState } from 'vuex'
import CurrentPage from 'theme/mixins/currentPage'
import AccountIcon from 'theme/components/core/blocks/Header/AccountIcon'
import CompareIcon from 'theme/components/core/blocks/Header/CompareIcon'
import HamburgerIcon from 'theme/components/core/blocks/Header/HamburgerIcon'
import Logo from 'theme/components/core/Logo'
import MicrocartIcon from 'theme/components/core/blocks/Header/MicrocartIcon'
import ReturnIcon from 'theme/components/core/blocks/Header/ReturnIcon'
import SearchIcon from 'theme/components/core/blocks/Header/SearchIcon'
import WishlistIcon from 'theme/components/core/blocks/Header/WishlistIcon'
import TopHeader from 'theme/components/core/blocks/TopHeader/TopHeader'

export default {
  name: 'Header',
  components: {
    AccountIcon,
    CompareIcon,
    HamburgerIcon,
    Logo,
    MicrocartIcon,
    ReturnIcon,
    SearchIcon,
    WishlistIcon,
    TopHeader
  },
  mixins: [CurrentPage],
  data () {
    return {
      navVisible: true,
      isScrolling: false,
      scrollTop: 0,
      lastScrollTop: 0,
      navbarHeight: 54,
      isMenuOpen: false
    }
  },
  computed: {
    ...mapState({
      isOpenLogin: state => state.ui.signUp,
      currentUser: state => state.user.current
    })
  },
  beforeMount () {
    window.addEventListener('scroll', () => {
      this.isScrolling = true
    }, {passive: true})

    setInterval(() => {
      if (this.isScrolling) {
        this.hasScrolled()
        this.isScrolling = false
        this.isMenuOpen = false
      }
    }, 250)
  },
  methods: {
    gotoAccount () {
      this.$bus.$emit('modal-toggle', 'modal-signup')
    },
    closeIconMenu() {
      this.isMenuOpen = false;
    },
    hasScrolled () {
      this.scrollTop = window.scrollY
      if (this.scrollTop > this.lastScrollTop && this.scrollTop > this.navbarHeight) {
        this.navVisible = false
      } else {
        this.navVisible = true
      }
      this.lastScrollTop = this.scrollTop
    }
  }
}
</script>

<style lang="scss" scoped>
@import '~theme/css/variables/colors';
@import '~theme/css/helpers/functions/color';
$color-icon-hover: color(secondary, $colors-background);

header {
  height: 65px;
  top: -76px;
  z-index: 2;
  padding-top: 10px;
  transition: top 0.2s ease-in-out;
  &.is-visible {
    top: 0;
  }
}
.icon {
  opacity: 0.6;
  &:hover,
  &:focus {
    background-color: $color-icon-hover;
    opacity: 1;
  }
}
.mobile-logo {
  max-width: none;
}
.right-icons {
  //for edge
  float: right;
}
.header-placeholder {
  height: 76px;
}
.links {
  text-decoration: underline;
}
.chevron-down,
.chevron-up {
  background: #f7f7f7;
  color: #676767;
  border: 0;
  font-size: 20px;
  height: 54px;
  width: 54px;
  line-height: 54px;
  position: relative;
  font-family: 'FontAwesome';
  &:before {
    top: 0;
    left: 0;
    right: 0;
    position: absolute;
    text-align: center;
  }
}
.chevron-down:before {
  content: '\f078';
}
.chevron-up:before {
  content: '\f077';
}
.icons-container {
  width: 100%;
  text-align: center;
  z-index: 1;
  position: absolute;
  top: 76px;
  background: #fff;
  span{
    display: block;
    position: absolute;
    top: 74px;
    left: 0;
    right: 0;
    height: 100%;
    width: 100%;
    background: rgba(0,0,0,0.6);
    min-height: 5000px;
  }
}

@media (max-width: 767px) {
  .row.middle-xs {
    margin: 0 -15px;

    &.py5 {
      margin: 0;
    }
  }
  .col-xs-2:first-of-type {
      padding-left: 0;
  }
  .col-xs-2:last-of-type {
      padding-right: 0;
  }
  a, span {
    font-size: 12px;
  }
  .micro-cart {
    display: none;
  }
}
</style>
